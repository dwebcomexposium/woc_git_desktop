      ...
      plugin.storeCurrentIntoHistory = function() {
         $element.find(plugin.settings.historyItem).each(function() {
            ...
            if(historyData!==null) { // Is there already some data ?
               ...
               // Insert ID at the beginning of the array
               if(json[data.historyType].indexOf(data.historyId)<0) json[data.historyType].unshift(data.historyId);
            } else { // New object
               json = {};
               json[data.historyType] = [data.historyId];
            }
